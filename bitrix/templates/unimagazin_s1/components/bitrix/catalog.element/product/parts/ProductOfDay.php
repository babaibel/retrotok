<?php
	...
	$monthCurrent = $dayCurrent->format('m');
	$dayCurrent = $dayCurrent->format('d');
	$monthEnd = date('m',strtotime($arResult["PROPERTIES"]["DAY_PROD"]["VALUE"]));
	$dayEnd = date('d',strtotime($arResult["PROPERTIES"]["DAY_PROD"]["VALUE"]));
	
	if ($monthCurrent == $monthEnd)
		if ($dayCurrent < $dayEnd)
			$enabled = true;
	
	unset($dayCurrent, $monthCurrent, $dayEnd, $monthEnd);
	...
	$month=$months[date("F",strtotime($arResult["PROPERTIES"]["DAY_PROD"]["VALUE"]))];
	$number=date("j",strtotime($arResult["PROPERTIES"]["DAY_PROD"]["VALUE"]));	
?>
<?if ($enabled == true || $arParams['PRODUCT_OF_DAY_SHOW_IF_END'] == 'Y'):?>
	<div class="row">
