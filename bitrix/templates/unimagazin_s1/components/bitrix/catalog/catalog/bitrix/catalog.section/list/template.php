		...
		?>
        	<div class="element hover_shadow" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
        		<?if(!empty($arElement["PREVIEW_PICTURE"]['ID'])){
        			$file = CFile::ResizeImageGet($arElement["PREVIEW_PICTURE"]['ID'],array('width'=>180, 'height'=>180),"BX_RESIZE_IMAGE_PROPORTIONAL_ALT");
					$src=$file['src'];
				}else if (!empty($arElement["DETAIL_PICTURE"]['ID'])){
					$file = CFile::ResizeImageGet($arElement["DETAIL_PICTURE"]['ID'],array('width'=>180, 'height'=>180),"BX_RESIZE_IMAGE_PROPORTIONAL_ALT");
        			$src=$file['src'];
				}else{
        		...
        		<a class="image-and-marks" href="<?=$arElement["DETAIL_PAGE_URL"]?>">
        			<div class="marks">
        		...
        		<div class="information">
                    ...
                    <?if (!empty($arElement["PREVIEW_TEXT"])):?>
        		        <div class="description"><?=html_entity_decode($arElement["PREVIEW_TEXT"])?></div>
                    <?endif?>
        			<div class="parameters">
        				<?$count=0;foreach($arElement['DISPLAY_PROPERTIES'] as $property){?>
        					<?if ( !is_array($property['VALUE'])) {?>
        					<div class="parameter"><?=$property['NAME'].': '.$property['DISPLAY_VALUE']?></div>
							<?} else {?>
							<div class="parameter"><?=$property['NAME'].': '.implode(',',$property['VALUE'])?></div>
							<?}?>
        				<?$count++;
        					if ($count == 3)
        					{
					...
					<?if ( $arElement['CAN_BUY'] || ($flg_offers && $flg_offers_can_buy)) {?>	
						<div class="buy">
							<a rel="nofollow" class="uni-button solid_button buy" id="buy_<?=$arElement['ID']?>"
								<?if(!$flg_offers){?>
									onclick="return add_to_cart(<?=$arElement['ID']?>,'<?=GetMessage("CATALOG_ADD_TO_BASKET")?>',this,<?=$arElement['CATALOG_MEASURE_RATIO']?>,'<?=$arParams["BASKET_URL"];?>','<?=$arElement['ID']?>','<?=$arParams['IBLOCK_ID']?>','<?=$arParams['IBLOCK_TYPE']?>')"
								<?}else{?>
