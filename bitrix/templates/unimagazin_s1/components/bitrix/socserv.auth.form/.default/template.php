...
<div class="bx-auth social">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
			<ul>
				<?foreach($arAuthServices as $service):?>
					<li><a href="javascript:void(0)" class="social-eshop" onclick="BxShowAuthService('<?=$service["ID"]?>', '<?=$arParams["SUFFIX"]?>')" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><span class="<?=htmlspecialcharsbx($service["ICON"])?>"></span></a></li>
				<?endforeach?>
			</ul>
		<?endif;?>		
